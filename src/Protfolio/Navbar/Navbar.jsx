    ...
    <nav className="navbar roboto max-w-7xl mx-auto bg-black flex justify-between items-center w-full">
      <div className="flex justify-between items-center w-full md:hidden">
        <div className="flex items-center">
              ...
              {navLinks.map((nav) => (
                  ...
                  >
                    ...
                    <Link to={`/${nav.id}`} className={`${
                      active === nav.title ? "text-primary":"text-white-300"}`}>{nav.title}</Link>
                    <span className={`${
                      active === nav.title ? "mt-[3px] absolute left-0 bottom-0 h-[4px] z-10 rounded-full bg-primary w-full"
          ...
          <a href='/home' className="text-xl font-bold">Abu Saiyed</a>
        </div>
        <button
      ...
      <div className="hidden md:flex justify-between items-center w-full">
        <a href='/home' className="text-2xl font-bold">Abu Saiyed</a>
        <ul className='list-none flex justify-center items-center gap-5'>
        {navLinks.map((nav) => (
            <li
              key={nav.id}
